...
class Peer extends EventEmitter {
    constructor({ id, roomId, displayName, socket }) {
        console.info('constructor() [id:"%s"]', id);
        super();
    ...
    _handlePeer() {
        if (this.socket) {
            this.socket.on('disconnect', () => {
                ...
                console.debug('"disconnect" event [id:%s]', this.id);
    ...
    addRole(newRole) {
        if (!this._roles.some((role) => role.id === newRole.id) &&
            ...
            console.info('addRole() | [newRole:"%s]"', newRole);
    ...
    removeRole(oldRole) {
        if (
            ...
            console.info('removeRole() | [oldRole:"%s]"', oldRole);
    ...
    addDataProducer(id, producer) {
        this.dataProducers.set(id, producer);
    }
    ...
    addDataConsumer(id, consumer) {
        this.dataConsumers.set(id, consumer);
    }
